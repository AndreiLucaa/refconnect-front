                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="font-medium truncate">
                                            {chat.description || `Chat ${chat.chatId.substring(0, 8)}`}
                                        </div>
                                        <div className="text-xs text-muted-foreground truncate">
                                            {chat.chatUsers?.length || 0} membri
                                        </div>
                                    </div>
                                </div>
                            </button>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold">
                                        {currentChat.description || `Chat ${currentChat.chatId.substring(0, 8)}`}
                                    </h3>
                                    <p className="text-xs text-muted-foreground">
                                        {currentChat.chatUsers?.length || 0} membri
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {messages.length === 0 ? (
                                ...
                                messages.map((msg: MessageDto) => (
                                    ...
                                    >
                                        <div className={`flex gap-2 max-w-[70%] ${msg.senderId === user?.id ? 'flex-row-reverse' : ''}`}>
                                            {/* Avatar */}
                                            <div className="h-8 w-8 rounded-full bg-secondary flex-shrink-0 overflow-hidden flex items-center justify-center">
                                                {msg.senderProfileImageUrl ? (
                                                    <img 
                                                        src={normalizeAssetUrl(msg.senderProfileImageUrl)} 
                                                        alt={msg.senderUserName} 
                                                        className="h-full w-full object-cover" 
                                                    />
                                                ) : (
                                                    <span className="text-xs">{msg.senderUserName?.charAt(0) || '?'}</span>
                                                )}
                                            </div>
                                            ...
                                            >
                                                {msg.senderId !== user?.id && (
                                                    <div className="text-xs font-semibold mb-1">{msg.senderUserName}</div>
                                                )}
